        ...
        register:function(event){
            ...
            if($('.reg input#email').val().match(/[\w\+\-\._]+@[\w\-\._]+\.\w{2,}/g)==null)
                ...
                $.post('/signup',reg_obj,function(data){
                    ...
                    if(temp.err==0)
                        $('div#reg_error').addClass('alert alert-success').html('<b>註冊成功.<br>歡迎使用Banvas行動名片夾!!<br>請至您的信箱收去認證信.</b>');
                    else if(temp.err==5)
                        $('div#reg_error').addClass('alert alert-error').html('<b>Email 已被人使用，請選擇另一個信箱!!</b>');
                });
                $('div#reg_error').empty();
            }
        }
